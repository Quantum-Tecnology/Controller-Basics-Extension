...
it('returns default per_page when blank', function () {
...
it('returns perPage if within max', function () {
    config(['page.per_page' => 15, 'page.max_page' => 50]);
    $support = new PaginationSupport();
    expect($support->calculatePerPage(10, 'foo'))->toBe(10);
});

it('caps perPage at max and logs when above max', function () {
    config(['page.per_page' => 15, 'page.max_page' => 50]);
    $support = new PaginationSupport();
...
it('returns 1 if perPage is falsy after all checks', function () {
